...
import { SafeAreaView, View, Text, TextInput, TouchableOpacity, Image, Alert, ScrollView } from "react-native";
        ...
        return (
          <View>
            ...
            <TouchableOpacity onPress={() => pickImage('recto')} style={{ justifyContent: 'center', backgroundColor: '#F5F5F5', borderColor: '#ccc', borderRadius: 5, height: 128, width: '100%', padding: 10, marginBottom: 10 }}>
              {images.recto ? (
                <Image source={{ uri: images.recto }} style={{ height: '100%', width: '100%' }} />
              ) : (
            ...
            <TouchableOpacity onPress={() => pickImage('verso')} style={{ justifyContent: 'center', backgroundColor: '#F5F5F5', height: 128, borderRadius: 5, padding: 10, marginBottom: 10 }}>
              {images.verso ? (
                <Image source={{ uri: images.verso }} style={{ height: '100%', width: '100%' }} />
              ) : (
        ...
        return (
          <View style={{ marginBottom: 24, alignItems: 'center' }}>
            <TouchableOpacity onPress={() => pickImage('profile')} style={{ position: 'relative', marginVertical: 24 }}>
              <View style={{ height: 148, width: 148, borderRadius: 74, borderWidth: 2, borderColor: '#ccc', overflow: 'hidden' }}>
                {images.profile ? (
  ...
  return (
    <SafeAreaView style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'white' }}>
      <ScrollView style={{ width: '100%', paddingHorizontal: 16 }}>
        ...
        {renderStepContent()}
        <View style={{ flexDirection: 'row', justifyContent: step === 1 ? 'center' : 'space-between', width: '100%', marginBottom: 24 }}>
          {step > 1 && (
            <TouchableOpacity
              onPress={() => setStep(step - 1)}
              style={{ backgroundColor: '#ccc', borderRadius: 5, padding: 10, width: 96 }}
            >
          ...
          {step < 4 ? (
            <TouchableOpacity
              ...
              style={{ backgroundColor: '#007AFF', borderRadius: 5, padding: 10, width: 96 }}
            >
